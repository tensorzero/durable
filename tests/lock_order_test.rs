...
async fn test_complete_run_with_lock_ordering(pool: PgPool) -> sqlx::Result<()> {
    let client = create_client(pool.clone(), "lock_complete")
        .register::<DoubleTask>()
        .unwrap()
        .build()
        .await
...
async fn test_fail_run_with_lock_ordering(pool: PgPool) -> sqlx::Result<()> {
    let client = create_client(pool.clone(), "lock_fail")
...
async fn test_sleep_for_with_lock_ordering(pool: PgPool) -> sqlx::Result<()> {
    let client = create_client(pool.clone(), "lock_sleep")
        .register::<SleepingTask>()
        .unwrap()
        .build()
        .await
        .unwrap();
...
async fn test_concurrent_complete_and_cancel(pool: PgPool) -> sqlx::Result<()> {
    let client = create_client(pool.clone(), "lock_conc_cc")
        .register::<SleepingTask>()
        .unwrap()
        .build()
        .await
...
async fn test_emit_event_with_lock_ordering(pool: PgPool) -> sqlx::Result<()> {
    ...
    let client = create_client(pool.clone(), "lock_emit")
...
async fn test_concurrent_emit_and_cancel(pool: PgPool) -> sqlx::Result<()> {
    ...
    let client = create_client(pool.clone(), "lock_emit_cancel")
        .register::<EventWaitingTask>()
        .unwrap()
        .build()
        .await
